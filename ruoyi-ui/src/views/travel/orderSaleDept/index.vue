    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['travel:orderSaleDept:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['travel:orderSaleDept:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['travel:orderSaleDept:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['travel:orderSaleDept:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="orderSaleDeptList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="订单id" align="center" prop="id" />
      <el-table-column label="下单产品id" align="center" prop="productId" />
      <el-table-column label="具体路线id" align="center" prop="routeId" />
      <el-table-column label="产品名称" align="center" prop="productName" />
      <el-table-column label="下单用户id" align="center" prop="userId" />
      <el-table-column label="下单手机号" align="center" prop="phonenumber" />
...
<script>
import { listOrderSaleDept, getOrderSaleDept, delOrderSaleDept, addOrderSaleDept, updateOrderSaleDept, exportOrderSaleDept } from "@/api/travel/orderSaleDept";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOrderSaleDept(this.queryParams).then(response => {
        this.orderSaleDeptList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOrderSaleDept(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateOrderSaleDept(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOrderSaleDept(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除订单门市部编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delOrderSaleDept(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有订单门市部数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportOrderSaleDept(queryParams);
        }).then(response => {
